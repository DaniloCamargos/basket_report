WITH parametros AS (
    SELECT
        '2025' AS ano_fixo,  -- Sempre 2025
        EXTRACT(MONTH FROM CURRENT_DATE()) AS mes_num_atual  -- Mês atual automático
),
...
JOIN parametros p
    ON m.ano_veiculacao = p.ano_fixo  -- Sempre 2025
JOIN mapa_meses mm
    ON mm.mes_nome = LOWER(m.mes_veiculacao)
   AND mm.mes_num = p.mes_num_atual  -- Mês atual automático
